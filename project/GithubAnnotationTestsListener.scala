...
class GithubAnnotationTestsListener(sourceDirs: Seq[File]) extends TestsListener {
    ...
    if (event.result.contains(TestResult.Failed)) {
      ...
      failed.foreach { t =>
        val line = if (t.throwable().isDefined) {
          ...
          println(s"::error file=${ele.getFileName},line=${ele.getLineNumber}::$firstLine")
          s"${ele.getClassName} ${ele.getFileName} ${ele.getMethodName} ${ele.getLineNumber}"
        }
  ...
  override def endGroup(name: String, t: Throwable): Unit = ()//println(s"endGroup($name, $t)")

  override def endGroup(name: String, result: TestResult): Unit = ()//println(s"endGroup($name, $result)")
}
